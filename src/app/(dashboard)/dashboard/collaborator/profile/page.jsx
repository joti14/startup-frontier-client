...
export default function CollaboratorProfile() {
    ...
    useEffect(() => {
        ...
        fetch(`${baseURL}/api/users/profile/${email}`)
            ...
            .then((data) => {
                ...
                if (data?.image) {
                    setImgState((s) => ({ ...s, preview: data.image, url: data.image }));
                }
                setIsLoading(false);
            });
    ...
    const handleImageChange = async (e) => {
        ...
        if (file.size > 2 * 1024 * 1024) {
            setGlobalError("Image must be under 2 MB.");
            return;
        }
        setGlobalError("");
    ...
    const onSubmit = async (data) => {
        ...
        setGlobalError("");
        try {
            const payload = { ...data, image: imgState.url || undefined };
    ...
    const uploadLabel = imgState.uploading
        ? "Uploading..."
        : imgState.fileName
            ? imgState.fileName.length > 28 ? imgState.fileName.slice(0, 28) + "..." : imgState.fileName
            : "Choose photo";

    return (
        <div className="max-w-2xl px-6 py-4">
            <DashboardHeading title="My Profile" description="Update your public collaborator profile." />
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <div className="relative">
                            <input
                                ...
                                className="w-full bg-white dark:bg-slate-900 text-slate-950 dark:text-white border border-slate-200 dark:border-slate-800 rounded-xl pl-9 pr-3.5 py-2 text-xs font-medium placeholder-slate-400 outline-none transition-all focus:border-slate-400 dark:focus:border-slate-600"
                            />
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex gap-2 items-center">
                            <div className="w-9 h-9 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center overflow-hidden shrink-0">
                                {imgState.preview ? (
                                    <img src={imgState.preview} alt="Preview" className="w-full h-full object-cover" />
                                ) : (
                            ...
                            <button
                                ...
                                className={`flex-1 px-3 py-2 text-xs font-bold border rounded-lg transition-all duration-200 text-left truncate shadow-sm flex items-center justify-between ${
                                    imgState.url
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <div className="relative">
                            <textarea
                                rows={4}
                                placeholder="Tell startups a bit about yourself, your experience, and what you're looking for..."
                                {...register("bio")}
                                className="w-full bg-white dark:bg-slate-900 text-slate-950 dark:text-white border border-slate-200 dark:border-slate-800 rounded-xl pl-9 pr-3.5 py-2 text-xs font-medium placeholder-slate-400 outline-none transition-all focus:border-slate-400 dark:focus:border-slate-600 resize-none"
                            />
                    ...
                    <button
                        type="submit"
                        disabled={isSubmitting || imgState.uploading}
