...
export default function Error({ error, reset }) {
    ...
    return (
        <section className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-transparent pb-24">

            {/* ─── BACKGROUND ─── */}
            <div className="absolute inset-0 z-0 pointer-events-none">
                <div className="absolute top-[-10%] left-1/2 -translate-x-1/2 w-[65rem] h-[55rem] rounded-full bg-red-200/20 dark:bg-red-900/10 blur-[130px]" />
            ...
            >
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 max-w-md mx-auto leading-relaxed font-medium">
                    Something broke on our end. Try again — it usually sorts itself out. If it keeps happening, head back home.
                </p>

                {/* Error message if available */}
                {error?.message && (
                    <div className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-red-100 dark:border-red-900/40 bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 text-xs font-mono max-w-sm mx-auto">
                        {error.message}
                    </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
                    <button
                        onClick={reset}
                        className="inline-flex items-center gap-2 rounded-full bg-slate-950 hover:bg-slate-900 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-50 text-white font-semibold text-xs tracking-wide h-11 px-8 shadow-md transition-all hover:scale-[1.01] cursor-pointer"
                    >
